      ...
      <div class="upload-container">
        ...
        <div class="upload">
          <el-upload
            v-show="uploadList.length == 0"
            drag
            action="http://175.178.161.86:9528/oss/uploadFile"
            multiple
            :before-upload="beforeUpload"
            :on-success="upload"
...
<script>
import { listPhoto,listPhotoid, getPhoto, delPhoto, addPhoto, updatePhoto, exportPhoto } from "@/api/system/photo";
import { listAlbum, getAlbum, delAlbum, addAlbum, updateAlbum, exportAlbum } from "@/api/system/album";
import * as imageConversion from "image-conversion";
export default {
  created() {
    console.log('id')
    console.log(this.$route.query.aitlbumId)
    this.getAlbumInfo();
    // this.listAlbums();
    this.listPhotos();
  },
  ...
  methods: {
    getAlbumInfo() {
      getAlbum(this.$route.query.aitlbumId).then(data =>{
    ...
    listPhotos() {
      this.queryParams.albumId = this.$route.query.aitlbumId
      listPhotoid(this.$route.query.aitlbumId).then(data =>{
        this.photoList = data.rows;
    ...
    console.log(this.$route.query.aitlbumId)
      addPhoto({
        albumId: this.$route.query.aitlbumId,
